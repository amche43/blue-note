...
class CommunityClient {
  ...
  static Json parse(String raw, {bool allowAnonymous = false}) {
    ...
    final uri = Uri.tryParse(
      value['url'] is String ? value['url'] as String : '',
    );
    if (uri == null ||
        uri.scheme != 'https' ||
    ...
    if ((!allowAnonymous &&
            (value['token'] is! String ||
                !RegExp(
                  r'^[a-f0-9]{64}$',
                ).hasMatch(value['token'] as String))) ||
        value['certificate'] is! String ||
        (value['certificate'] as String).length > 16000) {
      throw const FormatException('连接口令或证书不正确');
    }
    SecurityContext(
      withTrustedRoots: false,
    ).setTrustedCertificatesBytes(utf8.encode(value['certificate'] as String));
    return value;
  }
  ...
  Future<Json> request(String method, String path, [Json? data]) async {
    final security = SecurityContext(withTrustedRoots: false)
      ..setTrustedCertificatesBytes(
        utf8.encode(config['certificate'] as String),
      );
    ...
    try {
      final req = await client
          .openUrl(
            method,
            Uri.parse(config['url'] as String).replace(
      ...
      await for (final chunk in res.timeout(timeout)) {
        ...
        if (bytes.length > 20 * 1024 * 1024) {
          throw const FormatException('后台返回内容过大');
        }
      }
      final value = jsonDecode(utf8.decode(bytes));
      if (value is! Json) throw const FormatException('后台响应格式不正确');
      if (res.statusCode != 200) {
        throw FormatException(value['error'] as String? ?? '后台暂不可用');
      }
      return value;
    } on SocketException {
...
class FeedbackRepository {
  ...
  List<Json> get items => store.settings.entries
      .where((e) => e.key.startsWith('feedback:'))
      .map((e) => jsonDecode(e.value) as Json)
      .toList()
      .reversed
      .toList();
  Future<void> save(Json item) =>
      store.setting('feedback:${item['requestId']}', jsonEncode(item));
  Future<void> draft(String lesson, String title, String body) async {
    if (body.trim().isEmpty || body.length > 8000) {
      throw const FormatException('请填写反馈，最多 8000 字');
    }
    await save({
      'requestId': newId(),
      'lesson': lesson,
  ...
  Future<void> submit(Json item, CommunityClient client, String account) async {
    ...
    if (item['account'] != '' &&
        (item['account'] != account || item['server'] != url)) {
      throw const FormatException('请连接创建此提交记录时使用的后台和测试账户');
    }
    final bound = {...item, 'account': account, 'server': url};
    await save(
      bound,
    ); // Persist the retry identity before the request leaves the device.
    final reply = await client.request('POST', '/v1/feedback', {
      for (final key in ['requestId', 'lesson', 'title', 'body'])
        key: item[key],
    });
    if (reply['id'] is! String ||
        !['pending', 'reviewing', 'resolved'].contains(reply['status'])) {
  ...
  Future<void> refresh(CommunityClient client, String account) async {
    ...
    for (final remote in result['items'] as List) {
      final matches = items.where(
        (e) =>
            e['requestId'] == remote['request_id'] &&
            e['account'] == account &&
            e['server'] == client.config['url'],
      );
      if (matches.isNotEmpty &&
          ['pending', 'reviewing', 'resolved'].contains(remote['status'])) {
        await save({
          ...matches.first,
          'id': remote['id'],
...
class _CommunityPageState extends State<CommunityPage> {
  ...
  Future<void> reload() => run(() async {
    ...
    account = me['id'] as String;
    name = me['name'] as String;
    await feedback.refresh(client!, account!);
    if (!widget.feedbackOnly) {
      books = ((await client!.request('GET', '/v1/notebooks'))['items'] as List)
          .cast<Json>();
      final data = await client!.request(
        'GET',
        selectedBook == null ? '/v1/questions' : '/v1/notebooks/$selectedBook',
      );
  ...
  Future<void> configure() async {
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                maxLines: 5,
                decoration: const InputDecoration(labelText: '连接配置'),
              ),
            ],
          ),
        ),
    ...
    await run(() async {
      ...
      await widget.store.setting(
        'avatarImage',
        me['avatarImage'] as String? ?? '',
      );
      await widget.store.setting('avatar', '${me['avatar'] ?? 0}');
      client = next;
      account = me['id'] as String;
      name = me['name'] as String;
      questions = [];
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, update) => AlertDialog(
                ...
                children: [
                  ...
                  ...options.map(
                    (e) => ListTile(
                      ...
                      subtitle: Text(
                        (e.value['notebookTitle'] as String? ?? '').isEmpty
                            ? '未分本'
                            : '${e.value['notebookTitle']} · 第${e.value['questionNumber']}题',
                      ),
                      leading: Icon(
                        selected == e.key
                  ...
                  if (selected != null) ...[
                    ...
                    Text(
                      [
                          ...
                          .map((k) => widget.store.questions[selected]![k])
                          .where((v) => v != '')
                          .join('\n\n'),
                    ),
                  ],
          ...
          actions: [
            ...
            TextButton(
              onPressed: selected != null && consent
    ...
    await run(() async {
      final revisions =
          widget.store.events
              ...
              .toList()
            ..sort(compareEvents);
      await client!.request('POST', '/v1/questions', {
        'consent': true,
  ...
  Future<void> openReference() async {
    ...
    final raw = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('打开题目引用'),
        content: TextField(controller: input, maxLines: 3),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('取消'),
    ...
    await run(() async {
      final id = RegExp(
        r'blue-note:q:(user-[a-f0-9]{32})(?![a-f0-9])',
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text(widget.feedbackOnly ? '我的纠错反馈' : '学习社区 · 本机测试'),
      actions: [
        IconButton(
          tooltip: '连接设置',
        ...
        IconButton(
          tooltip: '刷新',
          onPressed: busy || client == null ? null : reload,
    ...
    body: ListView(
      ...
      children: [
        ...
        if (!widget.feedbackOnly) ...[
          FilledButton.icon(
            onPressed: busy || client == null
                ? null
                : () => Navigator.push<void>(
                    context,
                    MaterialPageRoute(
                      builder: (_) =>
                          HallPage(client: client!, store: widget.store),
                    ),
                  ),
          ...
          Wrap(
            ...
            children: [
              ...
              ...books.map(
                (b) => ChoiceChip(
                  label: Text('${b['title']} · ${b['name']} (${b['count']})'),
                  selected: selectedBook == b['id'],
                  onSelected: busy
                      ? null
                      : (_) async {
                          setState(() => selectedBook = b['id'] as String);
                          await reload();
                        },
                ),
              ),
          ...
          TextButton.icon(
            onPressed: busy || client == null ? null : openReference,
            icon: const Icon(Icons.link),
            label: const Text('粘贴题目引用'),
          ),
          ...
          if (!busy && questions.isEmpty)
            const Padding(
              ...
              child: Text('还没有共享题目。你的私人题目不会自动出现在这里。'),
            ),
          ...questions.map(
            (q) => Card(
              child: ListTile(
                title: Text(q['package']['question']['title'] as String),
                subtitle: Text(
                  '${q['package']['question']['notebookTitle'] ?? ''} ${(q['package']['question']['questionNumber'] ?? '') == '' ? '' : '第${q['package']['question']['questionNumber']}题'} · ${q['package']['author']} · ${q['likes']} 人点赞',
                ),
          ...
          if (feedback.items.isEmpty)
            const Padding(
              padding: EdgeInsets.all(20),
              child: Text('暂时没有反馈。在题目页点击“纠错反馈”即可填写。'),
            ),
          ...feedback.items.map(
            (f) => Card(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(feedbackLabels[f['status']] ?? '尚未提交'),
                    Text(f['body'] as String),
                    if ((f['reply'] as String).isNotEmpty)
                      Text('处理回复：${f['reply']}'),
                    if (f['status'] == 'draft')
                      TextButton(
                        onPressed: busy || account == null
                            ? null
                            : () => run(
                                () => feedback.submit(f, client!, account!),
                              ),
                        child: const Text('提交 / 重试'),
                      ),
                  ],
                ),
              ),
    ...
    builder: (ctx) => AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text(title),
            const Text('指出有疑问的步骤，也可以补充你的推导。先保存为“尚未提交”；提交时仅发送题目编号、标题和这段反馈。'),
            TextField(
              controller: controller,
              maxLines: 6,
              maxLength: 8000,
              decoration: const InputDecoration(labelText: '哪里有问题？'),
            ),
          ],
        ),
      ),
...
class _SharedQuestionPageState extends State<SharedQuestionPage> {
  ...
  Future<void> load() async {
    ...
    comments =
        ((await widget.client.request('GET', '$path/comments'))['items']
                as List)
            .cast<Json>();
  }
  ...
  Future<void> run(Future<void> Function() fn) async {
    ...
    try {
      ...
      if (mounted) error = e is FormatException ? e.message : '操作未确认完成，请稍后重试';
    } finally {
      if (mounted) setState(() => busy = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(q['contentKind'] == 'knowledge' ? '知识条目' : '学习条目'),
      ),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          ...
          TextButton(
            onPressed: question['ownerId'] == null
                ? null
                : () => Navigator.push<void>(
                    context,
                    MaterialPageRoute(
                      builder: (_) => LearningProfilePage(
                        ...
                        userId: question['ownerId'] as String,
                      ),
                    ),
                  ),
            child: Text('${question['package']['author']} · 查看学习履历'),
          ),
          const Text(
            '用户分享，内容尚未审核',
            style: TextStyle(fontSize: 12, color: Colors.blueGrey),
          ),
          if (question['mine'] != true)
            TextButton.icon(
              onPressed: busy
                  ? null
                  : () => run(() async {
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              value['value'] == true ? '已标记：帮我理解了' : '已取消帮助标记',
                            ),
                          ),
                        );
                      }
              ...
              label: const Text('帮我理解了 / 取消标记'),
            ),
          TextButton.icon(
            onPressed: busy
                ? null
                : () => run(() async {
                    question = await widget.client.request(
                      'PUT',
                      '$path/like',
            ...
            label: Text(
              '${question['liked'] == true ? '已点赞 · 点击取消' : '给这条内容点赞'} (${question['likes']})',
            ),
          ),

          if ((q['notebookTitle'] as String? ?? '').isNotEmpty)
            Text('${q['notebookTitle']} · 第${q['questionNumber']}题'),
          TextButton.icon(
            onPressed: busy
                ? null
                : () => run(() async {
                    await Clipboard.setData(
                      ClipboardData(
                        text:
                            '${q['notebookTitle'] ?? '共享题目'} 第${q['questionNumber'] ?? ''}题 · ${q['title']}\nblue-note:q:${question['id']}',
                      ),
                    );
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('已复制引用，可在同一后台的共享题库粘贴打开')),
                      );
                    }
                  }),
            icon: const Icon(Icons.link),
          ...
          ...[
              ...
              .where((k) => q[k] is String && q[k] != '')
              .map(
                (k) => Container(
                  margin: const EdgeInsets.symmetric(vertical: 12),
                  padding: const EdgeInsets.all(14),
                  decoration: BoxDecoration(
                    color: k == 'errorReason' || k == 'pitfall'
          ...
          FilledButton(
            onPressed: busy
                ...
                : () => run(() async {
                    ...
                    await widget.store.importQuestionPackage(
                      jsonEncode(fresh['package']),
                    );
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('已保存到私人题库，重复下载不会覆盖你的修改')),
                      );
                    }
                  }),
              ...
              onPressed: busy
                  ...
                  : () async {
                      final yes = await showDialog<bool>(
                        context: context,
                        builder: (ctx) => AlertDialog(
                          title: const Text('撤回这道题？'),
                          content: const Text('共享题库将不再显示。其他人已下载的副本仍会保留。'),
                          actions: [
                            TextButton(
                      ...
                      if (yes == true && mounted) {
                        await run(() async {
                          await widget.client.request('DELETE', path);
          ...
          TextField(
            ...
            maxLines: 3,
            maxLength: 2000,
            decoration: const InputDecoration(labelText: '说说你的理解'),
          ),
          TextButton(
            onPressed: busy
                ? null
                : () => run(() async {
                    if (comment.text.trim().isEmpty) {
                      throw const FormatException('请先填写评论');
                    }
                    await widget.client.request('POST', '$path/comments', {
                      'requestId': commentId,
          ...
          ...comments.map(
            (c) => ListTile(
              leading: BlueAvatar(
                photo: c['avatarImage'] as String?,
                index: c['avatar'] as int? ?? 0,
                width: 30,
              ),
              title: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: (c['body'] as String).split('\n').map((line) {
                  final index = stickerNames.indexWhere(
                    (name) => line == '[蓝笔表情:$name]',
                  );
                  return index < 0
                      ? Text(line)
