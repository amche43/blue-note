...
class StudyStore extends ChangeNotifier {
  ...
  static Future<StudyStore> open({DatabaseFactory? factory, String? path}) async {
    final selected = factory ?? databaseFactory;
    final location = path ?? paths.join(await selected.getDatabasesPath(), 'blue_note_v1.db');
    final db = await selected.openDatabase(location, options: OpenDatabaseOptions(
      version: 1,
      onCreate: (db, _) async {
        await db.execute('CREATE TABLE events (id TEXT PRIMARY KEY, lesson_id TEXT NOT NULL, type TEXT NOT NULL, at INTEGER NOT NULL, payload TEXT NOT NULL)');
        await db.execute('CREATE INDEX events_lesson ON events (lesson_id, at)');
  ...
  Future<void> refresh() async {
    events = (await db.query('events')).map((row) => StudyEvent(
      id: row['id'] as String, lessonId: row['lesson_id'] as String,
      type: row['type'] as String, at: row['at'] as int,
      payload: jsonDecode(row['payload'] as String) as Json,
    )).toList();
    settings = {for (final row in await db.query('settings')) row['key'] as String: row['value'] as String};
    questions = {};
    final revisions = events.where((e) => e.type == 'question').toList()..sort(compareEvents);
    for (final event in revisions) { questions[event.lessonId] = event.payload; }
    lessons = [...bundledLessons, ...questions.entries.where((e) => e.value['deleted'] == false)
      .map((e) => questionLesson(e.key, e.value))];
    notifyListeners();
  }

  Progress progress(String id) => Progress.fromEvents(events.where((e) => e.lessonId == id));
  List<Lesson> get due => lessons.where((l) {
    final at = progress(l.id).due;
    return at != null && !at.isAfter(DateTime.now());
  }).toList()..sort((a, b) => progress(a.id).due!.compareTo(progress(b.id).due!));
    ...
    await db.transaction((tx) async {
      ...
      for (final e in validated) {
        ...
        if (existing.isNotEmpty) {
          ...
          if (old['lesson_id'] != e.lessonId || old['type'] != e.type || old['at'] != e.at ||
              !mapEquals(jsonDecode(old['payload'] as String) as Json, e.payload)) {
            throw const FormatException('同一记录编号包含不同内容，已停止导入');
          }
          continue;
        }
        await tx.insert('events', {'id': e.id, 'lesson_id': e.lessonId, 'type': e.type, 'at': e.at, 'payload': encoded});
      }
    });
    await refresh();
  }

  Future<void> note(String lessonId, String text) => merge([StudyEvent(
    id: newId(), lessonId: lessonId, type: 'note', at: DateTime.now().millisecondsSinceEpoch,
    payload: {'text': text.trim()},
  )]);

  Future<void> attempt(String lessonId, {required String variantId, required String rating,
      required bool assisted, required bool correct, required String reason}) => merge([StudyEvent(
    id: newId(), lessonId: lessonId, type: 'attempt', at: DateTime.now().millisecondsSinceEpoch,
    payload: {'variantId': variantId, 'rating': rating, 'assisted': assisted, 'correct': correct, 'reason': reason},
  )]);
  ...
  Future<void> deleteQuestion(String id) async {
    ...
    await saveQuestion({...question, 'deleted': true}, id: id);
  }
  ...
  Future<String> importQuestionPackage(String raw) async {
    ...
    return saveQuestion({...data, 'origin': package['id'],
      'source': source.substring(0, source.length.clamp(0, 1000))});
  }
  Future<int> restore(String raw) async {
    final input = decodeBackup(raw); // Validate the entire file before the transaction.
  ...
  Future<int> sync(String address, String token) async {
    ...
    try {
      final request = await client.postUrl(uri.replace(path: '/v1/sync')).timeout(const Duration(seconds: 15));
      ...
      if (response.statusCode != 200) {
        throw HttpException(response.statusCode == 401 ? '同步口令不正确' : '服务暂不可用（${response.statusCode}）');
      }
      final bytes = <int>[];
      await for (final chunk in response.timeout(const Duration(seconds: 20))) {
        bytes.addAll(chunk);
        if (bytes.length > 8 * 1024 * 1024) throw const FormatException('同步数据超出首版上限');
      }
