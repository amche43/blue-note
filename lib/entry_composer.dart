...
class EntryComposer extends StatefulWidget {
  ...
  @override
  State<EntryComposer> createState() => _EntryComposerState();
}

class _EntryComposerState extends State<EntryComposer> {
  final title = TextEditingController(),
      prompt = TextEditingController(),
      breakthrough = TextEditingController(),
      thought = TextEditingController(),
  ...
  void initState() {
    ...
    draft = EditorDraft(
      widget.store,
      'editDraft:composer-${widget.initialKind}',
      () => {
        'fields': {for (final e in draftFields.entries) e.key: e.value.text},
        'kind': kind,
        'book': book,
        'step': step,
        'capture': capture,
    ...
    for (final c in draftFields.values) {
      ...
      try {
        final ready = await draft.recover(context, (data) {
          for (final e in (data['fields'] as Json).entries) {
            draftFields[e.key]?.text = e.value as String;
          }
          ...
          dirty = true;
          if (book.isNotEmpty && !notebooks(widget.store).containsKey(book)) {
            book = '';
          }
        });
        if (!mounted) return;
  ...
  Future<void> save() async {
    ...
    try {
      ...
      final id = await widget.store.saveQuestion(
        {
          ...
          'contentKind': kind,
          'action': breakthrough.text,
          'firstThought': thought.text,
          'errorReason': reason.text,
          'summary': summary.text,
          'notebookId': book,
          'notebookTitle': notebooks(widget.store)[book] ?? '',
        },
        capture: capture,
      ...
      if (mounted) {
        setState(() {
          busy = false;
          error = e is FormatException ? e.message : '保存未完成，请重试';
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) => PopScope<String>(
    ...
    child: Scaffold(
      appBar: AppBar(title: const Text('记录新的学习内容')),
      body: ListView(
        padding: const EdgeInsets.all(24),
        children: [
          draftStatus(draft),
          Text(
            ['1 · 记录内容', '2 · 写下自己的理解', '3 · 加入学习本'][step],
          ...
          if (step == 0) ...[
            ...
            OutlinedButton.icon(
              onPressed: busy
                  ...
                  : () async {
                      if (prompt.text.trim().isNotEmpty) {
                        final replace = await showDialog<bool>(
                          ...
                          builder: (ctx) => AlertDialog(
                            title: const Text('用识别结果替换现有正文？'),
                            actions: [
                              TextButton(
                                onPressed: () => Navigator.pop(ctx, false),
                                child: const Text('保留正文'),
                              ),
                              TextButton(
                      ...
                      final result = await Navigator.push<Json>(
                        context,
                        MaterialPageRoute(
                          builder: (_) => PhotoImportPage(store: widget.store),
                        ),
                      );
              ...
              label: const Text('拍照 / 相册识别'),
            ),
            DropdownButton<String>(
              value: kind,
              items: const [
                DropdownMenuItem(value: 'question', child: Text('错题 / 例题')),
                DropdownMenuItem(value: 'knowledge', child: Text('知识卡片')),
              ],
              onChanged: busy
                  ? null
                  : (v) => setState(() {
          ...
          if (step == 1) ...[
            ...
            ExpansionTile(
              ...
              children: [
                input(thought, '我的第一反应', lines: 2),
                if (kind == 'question') input(reason, '我的错误原因', lines: 2),
                input(summary, '一句话总结', lines: 2),
              ],
            ),
          ],
          if (step == 2) ...[
            DropdownButtonFormField<String>(
              initialValue: book,
              isExpanded: true,
              decoration: const InputDecoration(labelText: '把它放在哪里？'),
              items: [
                const DropdownMenuItem(value: '', child: Text('暂不分本，先保存')),
