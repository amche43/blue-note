...
class StudyEvent {
  ...
  final int at;
  final Json payload;
  StudyEvent({required this.id, required this.lessonId, required this.type, required this.at, required this.payload});
  Json toJson() => {'id': id, 'lessonId': lessonId, 'type': type, 'at': at, 'payload': payload};
  factory StudyEvent.fromJson(Json json) {
    final id = json['id'], lesson = json['lessonId'], type = json['type'], at = json['at'], payload = json['payload'];
    if (id is! String || !RegExp(r'^[a-f0-9]{32}$').hasMatch(id) ||
        lesson is! String || !RegExp(r'^[a-z0-9-]{1,80}$').hasMatch(lesson) ||
        !['note', 'attempt', 'question'].contains(type) || at is! int || at < 0 ||
        at > DateTime.now().add(const Duration(days: 1)).millisecondsSinceEpoch || payload is! Json) {
      throw const FormatException('学习记录格式不正确');
    }
    if (type == 'question') {
      if (!lesson.startsWith('user-') || !RegExp(r'^user-[a-f0-9]{32}$').hasMatch(lesson)) {
        throw const FormatException('自建题目编号不正确');
      }
      validateQuestion(payload);
    } else if (type == 'note') {
      if (payload['text'] is! String || (payload['text'] as String).length > 20000) {
        throw const FormatException('笔记格式不正确或过长');
      }
    } else {
      if (!['again', 'hard', 'good'].contains(payload['rating']) ||
          payload['assisted'] is! bool || payload['variantId'] is! String ||
          payload['correct'] is! bool || payload['reason'] is! String) {
        throw const FormatException('作答记录格式不正确');
      }
    }
    return StudyEvent(id: id, lessonId: lesson, type: type as String, at: at, payload: payload);
  }
}

/// One portable format for Android, future iOS, manual backups and the server.
List<StudyEvent> decodeBackup(String raw) {
  if (utf8.encode(raw).length > 8 * 1024 * 1024) throw const FormatException('备份超过 8MB');
  final data = jsonDecode(raw);
  if (data is! Json || ![1, 2].contains(data['schemaVersion']) || data['events'] is! List) {
